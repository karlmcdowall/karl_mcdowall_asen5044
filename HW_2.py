...
def plot_pertubations(timestamps: list[int], delta_x:list[np.ndarray]):
    ...
    delta_r = [delta[0] for delta in delta_x]
    ...
    line1, = ax1.plot(timestamps, delta_r, label=r"$\Delta r$", color=r_color, linewidth='0.5')

    ax2 = ax1.twinx()
    delta_r_dot = [delta[1] for delta in delta_x]
    ax2.yaxis.tick_left()
    ...
    line2, = ax2.plot(timestamps, delta_r_dot, label=r"$\Delta \dot{r}$", color=r_dot_color, linewidth='0.5')

    ax3 = ax1.twinx()
    delta_theta = [delta[2] for delta in delta_x]
    ...
    line3, = ax3.plot(timestamps, delta_theta, label=r"$\Delta\Theta$", color=theta_color, linewidth='0.5')
    ...
    line4, = ax4.plot(timestamps, delta_theta_dot, label=r"$\Delta \dot{\Theta}$", color=theta_dot_color, linewidth='0.5')
...
def plot_expected_total_system_state(timestamps: list[int], x_nom_plus_delta_x:list[np.ndarray]):
    # Make plots of pertubation-states vs time
    fig, ax1 = plt.subplots(figsize=(10, 5))
    ax1.set_xlabel("Time (s)")
    r_color, r_dot_color, theta_color, theta_dot_color = "red", "blue", "orange", "green"
    ...
    ax2.set_ylabel(r"Speed ($km s^{-1}$)", color=r_dot_color)
    ax2.tick_params(axis='y', labelcolor=r_dot_color)
    line2, = ax2.plot(timestamps, r_dot, label=r"$\dot{r}$", color=r_dot_color, linewidth='0.5')
    ...
    ax3.spines[["top", "left"]].set_visible(False)
    ax3.tick_params(axis='y', labelcolor=theta_color)
    line3, = ax3.plot(timestamps, theta, label=r"$\Theta$", color=theta_color, linewidth='0.5')
    ...
    ax4.spines[["top", "left"]].set_visible(False)
    ax4.tick_params(axis='y', labelcolor=theta_dot_color)
    line4, = ax4.plot(timestamps, theta_dot, label=r"$\dot{\Theta}$", color=theta_dot_color, linewidth='0.5')
...
def plot_rk4_total_system_state(timestamps: list[int], r:np.ndarray, r_dot: np.ndarray, theta: np.ndarray, theta_dot: np.ndarray):
    # Make plots of pertubation-states vs time
    fig, ax1 = plt.subplots(figsize=(10, 5))
    ax1.set_xlabel("Time (s)")
    r_color, r_dot_color, theta_color, theta_dot_color = "red", "blue", "orange", "green"
